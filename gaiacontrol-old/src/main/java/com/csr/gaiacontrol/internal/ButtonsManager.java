...
public class ButtonsManager {
     ...
     */
    public void askForAppVersion() {
        sendGaiaIamplusPacket(Gaia.GAIA_COMMAND_GET_FW_VERSION);
        sendGaiaPacket(Gaia.COMMAND_GET_APPLICATION_VERSION);
    }
     ...
     */
    protected void handlePacket(Message msg) {
        ...
        if (packet.getVendorId() == Gaia.VENDOR_CSR) {
            ...
            switch (packet.getCommandId() & 0x0FFF) {
                ...
                case Gaia.GAIA_COMMAND_GET_FW_VERSION:
                    validate = checkStatus(packet);
                    if (DEBUG)
                        Log.i(TAG, "Received IAMPLUS vendor \"GAIA_COMMAND_GET_FW_VERSION\" packet with a " + validate + " status.");
                    receiveGetAPPVersion(packet);
    ...
    private void receiveGetAPPVersion(GaiaPacket packet) {
        try {
            ...
            Log.e(TAG, "receiveGetAPPVersion: ", e);
        }
    }
    ...
    private void receivePacketCommandNotSupported(GaiaPacket packet) {
        ...
        if (packet.getVendorId() == Gaia.VENDOR_IAMPLUS) {
            switch (packet.getCommand()) {
                ...
                case Gaia.GAIA_COMMAND_GET_FW_VERSION:
                    if (DEBUG)
                        Log.w(TAG, "Received \"GAIA_COMMAND_GET_FW_VERSION\" not supported.");
                    break;
            }
        } else {
            switch (packet.getCommand()) {
    ...
    protected void notifyOnError(GaiaError error) {
        for (Callback callback : mListener) {
